...
def build_scrum_graph():
    ...
    # **FIX**: Add the initial progress broadcast here
    ...
    # ... (wait_command, node_ask_update remain the same)
    async def wait_command(state: ScrumState): return {}
    ...
    # **FIX**: Re-fetch the task list after every update
    async def node_collecting(state: ScrumState):
        ...
        if hasattr(pm, 'add_comment') and result.get("need_comment"): 
            pm.add_comment(task["id"], comment)
        if hasattr(pm, 'update_task_status'): 
            pm.update_task_status(task["id"], status)
        
        # After updating, fetch the fresh list from the Project Manager.
        ...
        # Return the updated_tasks list in the state.
        return {"current_task": idx + 1, "state": "evaluating_next_task", "tasks": updated_tasks}

    # ... (rest of the file is identical to the previous working version)
    ...
    def placeholder_should_continue(state: ScrumState): return {}
    def route_start(state: ScrumState):
        current_state = state.get("state", "INIT")
        if current_state == "INIT": return "init"
        elif current_state == "wait_command": return "wait_command"
        elif current_state == "collecting": return "collecting"
        elif current_state == "evaluating_next_task": return "should_continue_router"
        else: return END
    def route_after_wait_command(state: ScrumState):
        last_input = state.get("_last_input")
        if not last_input: return END
        if (last_input.get("message") or "").strip().lower() == "start": return "ask_update"
        else: return "prompt_for_start"
    def should_continue(state: ScrumState):
        tasks = state.get("tasks", []); current_task_idx = state.get("current_task", 0)
        return "ask_update" if current_task_idx < len(tasks) else "summary"
    g.add_node("init", init); g.add_node("show_tasks", node_show_tasks); g.add_node("wait_command", wait_command)
    g.add_node("prompt_for_start", prompt_for_start); g.add_node("ask_update", node_ask_update); g.add_node("collecting", node_collecting)
    g.add_node("summary", summary); g.add_node("should_continue_router", placeholder_should_continue)
    g.add_conditional_edges(START, route_start, {"init": "init", "wait_command": "wait_command", "collecting": "collecting", "should_continue_router": "should_continue_router", END: END})
    g.add_edge("init", "show_tasks"); g.add_edge("show_tasks", "wait_command")
    g.add_conditional_edges("wait_command", route_after_wait_command, {"ask_update": "ask_update", "prompt_for_start": "prompt_for_start", END: END})
    g.add_edge("prompt_for_start", "wait_command"); g.add_edge("ask_update", END); g.add_edge("collecting", END)
    g.add_conditional_edges("should_continue_router", should_continue, {"ask_update": "ask_update", "summary": "summary"})
