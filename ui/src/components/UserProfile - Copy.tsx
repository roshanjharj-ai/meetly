...
const UserProfile = () => {
    ...
    const { user, setUser, toggleTheme, theme } = useContext(UserContext);
    
    // **FIX**: Get the 'reset' function from useForm
    ...
    const handleCropSave = () => {
        // ... (handleCropSave function remains the same)
        if (completedCrop && imgRef.current && canvasRef.current) {
            const image = imgRef.current; const canvas = canvasRef.current;
            const scaleX = image.naturalWidth / image.width; const scaleY = image.naturalHeight / image.height;
            const pixelRatio = window.devicePixelRatio;
            canvas.width = completedCrop.width * pixelRatio * scaleX; canvas.height = completedCrop.height * pixelRatio * scaleY;
            const ctx = canvas.getContext('2d');
            if (!ctx) return;
            ctx.setTransform(pixelRatio, 0, 0, pixelRatio, 0, 0); ctx.imageSmoothingQuality = 'high';
            ...
            setValue('picture', base64Image, { shouldDirty: true }); setShowCropModal(false);
        }
    };
    ...
    const onSubmit = async (data: ProfileFormData) => {
        ...
        try {
            ...
            if (setUser) setUser(prev => prev ? ({ ...prev, ...updatedUser }) : updatedUser);
    ...
    return (
        <>
            {/* ... (style tag and main layout div remain the same) ... */}
            <style>{` .dropzone { /*...*/ } .dropzone:hover, .dropzone-active { /*...*/ } .cropper-modal-backdrop { /*...*/ } .cropper-modal-content { /*...*/ } [data-bs-theme="light"] .cropper-modal-content { /*...*/ } .ReactCrop__image { /*...*/ } `}</style>
            <div className="container-fluid d-flex justify-content-center align-items-center" style={{ minHeight: 'calc(100vh - 56px)', padding: '40px 15px' }}>
                <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className="card card-body p-4 rounded-4 border-0 shadow-lg" style={{ minWidth: '320px', maxWidth: '500px', width: '90%' }}>
                    ...
                    {/* ... (form and the rest of the component remains the same) ... */}
